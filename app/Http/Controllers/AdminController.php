...
class AdminController extends Controller
{
    ...
    public function show_dashboard(Request $request)
    {
        ...
        $product_views = Product::orderBy('product_views', 'DESC')
            ->take(20)
            ->get();
        $post_views = Post::orderBy('post_views', 'DESC')
            ->take(20)
            ->get();
        return view('admin.dashboard')->with(compact('visitors_total', 'visitor_count', 'visitor_last_month_count', 'visitor_this_month_count', 'visitor_year_count', 'product', 'posts', 'order', 'video', 'customer', 'product_views', 'post_views'));
    }
    ...
    public function callback_customer_google() {
        ...
        $users = Socialite::driver('google')->stateless()->user();
            ...
            return redirect('/')->with('message', 'Đăng nhập tài khoản google <span style="color: red">
            '.$account_name->customer_email.'</span>thành công');
    }
    ...
    public function dashboard_filter(Request $request)
    {
        ...
        $tomorrow = Carbon::now('Asia/Ho_Chi_Minh')
            ->addDay()
            ->format('d-m-Y H:i:s');
        $lastWeek = Carbon::now('Asia/Ho_Chi_Minh')
            ->subWeek()
            ->format('d-m-Y H:i:s');
        $sub15days = Carbon::now('Asia/Ho_Chi_Minh')
            ->subdays(15)
            ->format('d-m-Y H:i:s');
        $sub30days = Carbon::now('Asia/Ho_Chi_Minh')
            ->subdays(30)
            ->format('d-m-Y H:i:s');

        //subMonth là trừ đi 1 tháng
        $dauthangnay = Carbon::now('Asia/Ho_Chi_Minh')
            ->startOfMonth()
            ->toDateString();
        $dau_thangtruoc = Carbon::now('Asia/Ho_Chi_Minh')
            ->subMonth()
            ->startOfMonth()
            ->toDateString();
        $cuoi_thangtruoc = Carbon::now('Asia/Ho_Chi_Minh')
            ->subMonth()
            ->endOfMonth()
            ->toDateString();

        $sub7days = Carbon::now('Asia/Ho_Chi_Minh')
            ->subdays(7)
            ->toDateString();
        $sub365days = Carbon::now('Asia/Ho_Chi_Minh')
            ->subdays(365)
            ->toDateString();

        $dauthang10 = Carbon::now('Asia/Ho_Chi_Minh')
            ->subMonth()
            ->startOfMonth()
            ->toDateString();
        $cuoithang10 = Carbon::now('Asia/Ho_Chi_Minh')
            ->subMonth()
            ->endOfMonth()
            ->toDateString();
        ...
        if ($data['dashboard_value'] == '7ngay') {
            ...
            $get = Statistic::whereBetween('order_date', [$dauthang10, $cuoithang10])
                ->orderBy('order_date', 'ASC')
                ->get();
        } else {
    ...
    {
        $sub60days = Carbon::now('Asia/Ho_Chi_Minh')
            ->subdays(30)
            ->toDateString();
